...
interface BackendAssignedGameResponse {
    ...
    game: { activityId: number; activityName: string; description?: string; subject?: string; };
    deadline: string;
    isPremade: boolean;
}
...
export const ClassroomProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const transformBackendTeacherClassroomToFrontend = useCallback((dto: BackendTeacherClassroomResponse): Classroom => {
    return {
      id: dto.id.toString(),
      name: dto.name,
  ...
  const fetchTeacherClassrooms = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch(`${CLASSROOM_API_BASE_URL}/teacher/${currentUser.id}`, {
      ...
      setTeacherClassrooms(data.map(transformBackendTeacherClassroomToFrontend));
    } catch (error) {
  ...
  const fetchStudentClassrooms = useCallback(async () => {
    ...
    try {
      ...
      const backendClassrooms: BackendTeacherClassroomResponse[] = await response.json();
  ...
  const createClassroom = useCallback(
    async (data: CreateClassroomRequestDTO): Promise<Classroom | null> => {
        ...
        try {
          ...
          const newBackendClassroom: BackendTeacherClassroomResponse = await response.json();
          const newClassroom = transformBackendTeacherClassroomToFrontend(newBackendClassroom);
...
const joinClassroom = useCallback(
  async (code: string): Promise<StudentClassroom | null> => {
      if (!currentUser || !token || currentUser.role !== 'STUDENT') {
        throw new Error("User must be a logged-in student to join a classroom.");
      }
      setIsLoading(true);
      try {
        ...
        const joinedBackendClassroom: BackendTeacherClassroomResponse = await response.json();
  ...
  const getAssignedGames = useCallback(
    async (classroomId: string): Promise<AssignedGameDTO[]> => {
        ...
        try {
          ...
          const backendGames: BackendAssignedGameResponse[] = await response.json();

          const assignedGamesFrontend: AssignedGameDTO[] = backendGames.map(bg => ({
            id: bg.id.toString(),
    ...
    async (classroomId: string): Promise<User[]> => {
        ...
        try {
          ...
          return studentsData.map(student => ({
            ...
            role: student.role,
            avatarUrl: (student as any).avatarImage || (student as any).profilePhoto || `https://api.dicebear.com/7.x/bottts/svg?seed=${student.firstName}${student.lastName}`,
          }));
        } catch (error) {
  ...
  const assignGameToClassroom = useCallback(
    async (classroomId: string, gameId: string, deadline: string, isPremade: boolean): Promise<AssignedGameDTO | null> => {
        ...
        try {
          ...
          const assignedBackendGame: BackendAssignedGameResponse = await response.json();
  ...
  const getClassroomLeaderboard = useCallback(
    async (classroomId: string): Promise<LeaderboardEntry[]> => {
        ...
        try {
          ...
          const studentsWithScores: BackendLeaderboardEntry[] = await response.json();
          const leaderboardEntries: LeaderboardEntry[] = studentsWithScores
            ...
            .map((student, index) => ({
                ...
                rank: index + 1,
                avatarUrl: student.avatarImage || `https://api.dicebear.com/7.x/bottts/svg?seed=${student.firstName}${student.lastName}`,
          }));
