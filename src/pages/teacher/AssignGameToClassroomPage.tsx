...
const PREFERRED_LIBRARY_GAMES: LibraryGameDisplayConfig[] = [
  {
    ...
    title: "Matching Game",
    description:
      "Test memory by matching pairs. Covers Anyong Tubig, Anyong Lupa, and Pambansang Sagisag.",
    gameMode: "MATCHING",
  ...
  },
  {
    ...
    title: "Sorting Game",
    description:
      "Create interactive sorting activities where students categorize items. Perfect for teaching classification.",
    gameMode: "SORTING",
    ...
    const fetchAndProcessGames = async () => {
      ...
      try {
        ...
        PREFERRED_LIBRARY_GAMES.forEach((preferredConfig) => {
          ...
          if (
            matchingBackendGame &&
            !processedBackendGameIds.has(matchingBackendGame.id)
          ) {
            finalGamesList.push({
            ...
            if (
              !finalGamesList.some(
                (fg) =>
                  fg.config.idKey === preferredConfig.idKey &&
                  !fg.actualGameDataFromBackend
              )
            ) {
              finalGamesList.push({
                config: preferredConfig,
        ...
        allLibraryGames.forEach((backendGame) => {
          if (!processedBackendGameIds.has(backendGame.id)) {
            const genericConfig: LibraryGameDisplayConfig = {
              ...
              description:
                backendGame.description || "No description available.",
              subject: backendGame.subject,
              gameMode: backendGame.gameMode,
              icon: PREFERRED_LIBRARY_GAMES.find(
                (pc) => pc.gameMode === backendGame.gameMode
              )?.icon || <Library size={28} className="text-gray-500" />,
            };
            finalGamesList.push({
        ...
        setDisplayableGames(
          PREFERRED_LIBRARY_GAMES.map((config) => ({
            config,
            actualGameDataFromBackend: undefined,
          }))
        );
  ...
  const handleOpenAssignModal = (gameDataForAssignment?: GameDTO) => {
    if (gameDataForAssignment) {
      ...
      setMessageModal({
        ...
        message:
          "This game configuration is not available for assignment or is missing data.",
        type: "error",
      });
    }
  };
          ...
          <nav
            ...
            {PREFERRED_LIBRARY_GAMES.map((gameConfig) => (
              <button
                ...
                onClick={() =>
                  gameConfig.gameMode && handleTabClick(gameConfig.gameMode)
                }
                className={`
                  ${
        ...
        {filteredGames.length === 0 && !isLoading ? (
          <div className="text-center py-10">
            <p className="text-xl text-gray-500 dark:text-gray-400">
              {activeTab === "all"
                ? "No games available in the library yet."
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filteredGames.map(({ config, actualGameDataFromBackend }) => (
              <div
                  ...
                  <div className="w-full space-y-1 mb-4">
                    {config.subject && (
                      <span className="inline-block bg-primary-accent/20 dark:bg-primary-accent-dark/30 text-primary-accent dark:text-primary-accent-dark text-xs font-semibold mr-2 px-2.5 py-1 rounded-full">
                        <BookCopy size={12} className="inline mr-1.5" />
                        {config.subject}
                      </span>
                    )}
                    {config.gameMode && (
                ...
                <Button
                  onClick={() =>
                    handleOpenAssignModal(actualGameDataFromBackend)
                  }
                  variant="primary"
                  fullWidth
                  className={`mt-auto ${
                    !actualGameDataFromBackend
